...
allPR = zeros(numSims,1); %proportion relapse
...
for i=1:length(good_indices)
    ...
    estimated_incidence = XX{k, 5};


   % subplot(2,2,3)
   plot(estimated_incidence)
end
hold off
...
for i=1:length(good_indices)
    ...
    PR=  sum(u*XELTR(:,5))/sum(p*w*XELTR(:,2)+v*XELTR(:,3)+u*XELTR(:,5)) ;
...
LTBI_newImmigrants = allq1(good_indices)+allq2(good_indices) + allq3(good_indices);
...
subplot(2,2,3)
scatter(allq1,allq2+allq3,'DisplayName','(qL+qR) vs qE')
hold on
...
TOL = 1.0;
indices_Houben = find(LTBI_newImmigrants<=upper_cutoff & LTBI_newImmigrants>=lower_cutoff & alls(good_indices)>1e-3);
