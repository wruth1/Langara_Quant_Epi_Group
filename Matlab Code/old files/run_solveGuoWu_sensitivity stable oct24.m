...
f=@(x)IncidenceError(x,bioParameters, initialConditions, ReportedImmigration, ReportedTB);
...
[xmin,fval,exitflag,output,lambda,grad,hessian] = fmincon(f, x0 , A , b, Aeq, beq, lb, ub) ; % about 10 seconds to run

% calculate 
[BP2, IC2] = updateParameters(bioParameters, initialConditions, xmin);
[XELTR, EstimatedIncidence] = solveGuoWu(BP2, IC2, ReportedImmigration);


% figure 
% % plot incidence of xmin
...
beta_list = [beta, beta*2];
p_list = [p/2,p,p*2];
w_list = [w/2,w,2*w];

v_list = [v/4, v/2, v, 2*v];            % The rate of slow progression to active TB due to reactivation
a_list = [a/2,a];              % TB-caused death rate
d_list = [d/2,d,d*2];               % Constant rate of recovery by nature or treatment
n_list = [n]; 
...
for i = 1:NumSims
    ...
    f=@(x)IncidenceError(x,BPi, ICi, ReportedImmigration, ReportedTB);
...
function [BP, IC] = updateParameters(BP0, IC0, x4)
    BP = BP0;
    IC = IC0;
...
function err = IncidenceError(x, bioParameters, initialConditions, ImmigrationRate, ReportedTB)
% input: x is in R4, q1, q2, E0, L0
...
function compareIncidence(x, bioParameters, initialConditions, ImmigrationRate, ReportedTB, yearGrid )
    ...
    % load E0
    % load L0
    initialConditions(2)=x(3);
    initialConditions(3)=x(4);

    [XELTR, EstimatedTB] = solveGuoWu(bioParameters, initialConditions, ImmigrationRate);


    plotPopulationVsTime(XELTR)

    subplot(2,3,6)    
    plot(EstimatedTB, 'DisplayName','Estimated Incidence')
    hold on
    plot(ReportedTB, 'DisplayName','Reported Incidence')
    grid on;
    title('Incidence ');
