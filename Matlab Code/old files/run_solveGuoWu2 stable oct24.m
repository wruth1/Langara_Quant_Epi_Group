...
initialConditions = [TP0, E0, ReportedTB(1), T0, R0 ];


[XELTR, EstimatedIncidence] = solveGuoWu2(bioParameters, initialConditions, ReportedImmigration);
...
f4=@(x)IncidenceError4(x,bioParameters, initialConditions, ReportedImmigration, ReportedTB);
...
[xmin4,fval4,exitflag,output,lambda,grad,hessian] = fmincon(f4, x0 , A , b, Aeq, beq, lb, ub) ; % about 10 seconds to run


% calculate 
[BP2, IC2] = updateParameters4(bioParameters, initialConditions, xmin4);
[XELTR, EstimatedIncidence] = solveGuoWu2(BP2, IC2, ReportedImmigration);
...
xcomparison = zeros(length(xmin4), 2);
xcomparison(:,1) = x0;
xcomparison(:,2) = xmin4;
...
% minimizer starting value
x0 = xmin4(4); % load L0 from 5D solve
...
for i = 1:NumSims
    ...
    f4=@(x)IncidenceError(x,BPi, ICi, ReportedImmigration, ReportedTB);
...
function [BP, IC] = updateParameters4(BP0, IC0, x4)
    BP = BP0;
    IC = IC0;
...
function err = IncidenceError4(x, bioParameters, initialConditions, ImmigrationRate, ReportedTB)
% input: x is in R5, q1, q2, E0, R0
...
[~, EstimatedTB] = solveGuoWu2(localBP, localIC, ImmigrationRate);
