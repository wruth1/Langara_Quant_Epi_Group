function [XELTR, TBIncidence] = solveGuoWu2(bioParameters, initialConditions, ImmigrationRate)
%{
  Input: 
    - bioParameters vector in R9, hard coded so position [beta p w v a d n q1 q2]
    - initialConditions vector in R5, hard coded so position [TP0 E0 incidence0 T0
    R0]. TP0 is total population, and used to calculate X0; incidence0 used
    to calculate L0.
...
% solve for L: (100 000 / total population) * (pwE + vL) = incidence rate)
L0 = (initialConditions(3)*initialConditions(1)/1e5-p*w*E0)/v;
